...
class Command(BaseCommand):
    ...
    def _setup_language_redirection(self):
        ...
        root = SignUpPage(
            ...
            slug="signin",
            welcome_message="Hi",
            content_type=ContentType.objects.get_for_model(SignUpPage),
            show_in_menus=True,
            locked=True,
            depth=1,
            path='/'
    ...
    def _setup_home(self):
        ...
        for language_code, label in settings.LANGUAGES:
            if language_code == "de":
                ...
                raise RuntimeError(
                    f"unsupported language encountered: {language_code}")
            homepage = HomePage(
    ...
    def _setup_default_pages(self):
        defaultpage_content_type, __ = ContentType.objects.get_for_model(
            DefaultPage)
    ...
    def _setup_contact_page(self):
        ...
        contact_page_content_type = ContentType.objects.get_for_model(
            ContactPage)
        home_page_de = HomePage.objects.get(language="de")
    ...
    def _setup_project_pages(self):
        ...
        project_page_content_type = ContentType.objects.get_for_model(
            ProjectPage)
    ...
    def _create_main_menu(self):
        ...
        if not main_menu_de.get_menu_items_manager().exists():
            ...
            contact_page_de = ContactPage.objects.descendant_of(
                home_page_de).first()
            item_list.append(
                item_class(
                    menu=main_menu_de,
        ...
        if not main_menu_en.get_menu_items_manager().exists():
            ...
            contact_page_en = ContactPage.objects.descendant_of(
                home_page_en).first()
            item_list.append(
                item_class(
                    menu=main_menu_en,
